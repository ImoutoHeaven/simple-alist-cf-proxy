...
func TestFlowDetachVisibleKeepsBucketWRROrderAcrossReattach(t *testing.T) {
	...
	reqDetached := atomicBreakerAcquireRequest("example.com", "h1", "ip-detached", "s1")
	tokDetached := store.newFlowFromAcquireRequest(reqDetached)
	tokOther := store.newFlow("h1", "example.com", "ip-other", "s1")
	leaseUntil := now.Add(30 * time.Second)

	resp, err := store.acceptAcquireInvocation(tokDetached, reqDetached, &fqWaiter{resCh: make(chan *AcquireResponse, 1)}, now, leaseUntil, inFlightLimits{})
	if err != nil {
		t.Fatalf("acceptAcquireInvocation detached flow err=%v", err)
	}
	if resp == nil || resp.Result != "pending" {
		t.Fatalf("expected accepted pending detached flow before reconnect transition, got %+v", resp)
	}
	...
	if !store.detachToReconnectWindow(tokDetached, now) {
	...
	reattachResp, err := store.acceptAcquireInvocation(tokDetached, reqDetached, &fqWaiter{resCh: make(chan *AcquireResponse, 1)}, now.Add(3*time.Millisecond), now.Add(30*time.Second), inFlightLimits{})
	if err != nil {
		t.Fatalf("reattach detached flow err=%v", err)
	}
	if reattachResp == nil || reattachResp.Result != "pending" {
		t.Fatalf("expected reattach to return pending, got %+v", reattachResp)
	}
...
func TestFlowDetachVisibleKeepsBucketWRROrderAcrossHostWideGap(t *testing.T) {
	...
	if resp == nil || resp.Result != "pending" {
		t.Fatalf("expected accepted pending detached flow before host-wide gap, got %+v", resp)
	}
	if ok, err := store.attachWaiter(tokOther, &fqWaiter{resCh: make(chan *AcquireResponse, 1)}, now); !ok || err != nil {
		t.Fatalf("attachWaiter other flow ok=%t err=%v", ok, err)
	}
	...
	if !store.detachToReconnectWindow(tokOther, now) {
	...
	reattachResp, err := store.acceptAcquireInvocation(tokDetached, reqDetached, &fqWaiter{resCh: make(chan *AcquireResponse, 1)}, now.Add(2*time.Millisecond), now.Add(30*time.Second), inFlightLimits{})
	if err != nil {
		t.Fatalf("reattach detached flow err=%v", err)
	}
	if reattachResp == nil || reattachResp.Result != "pending" {
		t.Fatalf("expected detached reattach to return pending, got %+v", reattachResp)
	}
