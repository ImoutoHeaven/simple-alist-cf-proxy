...
- PostgREST + PostgreSQL：ticket state、下载缓存协调、限流与 Breaker 的权威状态与 RPC 入口
...
- `download.db.mode` 支持 `custom-pg-rest` 下载协调模式；下载请求要求该模式与 `cacheEnabled=true`
- `download.db.*`：PostgREST 地址、校验 header/secret、`cacheEnabled`、缓存表/ticket-state 表、TTL 等；`custom-pg-rest` 模式提供完整 ticket-state 与下载缓存 wiring
...
4. **payload/payloadSign 校验**
    ...
    - 解码 `payload`，始终要求 `expireTime` 合法；下载协调模式额外要求 `ticketNonce` 与 `idle_timeout` 存在且格式合法。
...
7. **统一检查（custom-pg-rest）**
     ...
     - 同时返回缓存命中、限流状态与 `THROTTLE_PROTECTION` 里的 breaker 原始快照；RPC 会原子更新限流计数与窗口，并读取缓存和 breaker 权威状态；worker 不生成本地 breaker 状态；`pgErrorHandle` 支持 `fail-open`/`fail-closed`。
...
8. **Ticket State admission**
    ...
    - 下载协调器配置缺失、停用或凭据无效时，请求在 AList 获取前返回带 `Retry-After` 的 `503`。

9. **缓存与 AList 获取**
    - 优先使用 unified-check 或 cacheManager 的有效缓存；未命中时取得单路径刷新租约，只有 owner 请求 AList `/api/fs/link`，followers 只读轮询租约状态。
    - owner 在打开并验证有效 `2xx` 内容后等待条件缓存发布；过期或替换的租约无法发布，失败结果共享 30 秒退避。
...
10. **Breaker 保护**
     ...
     - 下载后的响应矩阵固定为：`2xx` 才会继续代理 upstream body；`3xx` 保持当前 redirect / deferred-report 路径；`4xx`/`5xx` 在 redirect 与 deferred flush 完成后、任何 CQ managed stream 或普通 `new Response(response.body, ...)` 之前统一进入 terminal classifier，先做 breaker bookkeeping、CQ release 与 fairqueue cleanup，再返回保留原 upstream status 的 Worker-generated JSON error envelope，字段为 `status`、`message`、`reason`，可带 `upstream_status` 与 `retry-after`。
     - `protectHttpCodes` 命中的 terminal `4xx`/`5xx` 一般走 `download_report_breaker_sample(sample=1)`；已验证为 Google quota 的 terminal 错误走 `download_settle_breaker_attempt` 结清 no-sample debt，保护列表之外的 terminal `4xx`/`5xx` 也在 live attempt 存在时走该路径。`2xx/3xx` 仍是 `sample=0` 的唯一路径；`Retry-After` 只解析数值秒，先做 cap，再把非数值场景交给 SQL 里的指数回退；受保护的 `half_open` attempt 仍会立即重新 `open`。
...
13. **上游请求与响应封装**
     - 支持 3xx 重定向；缓存刷新 owner 按 provider、授权大小与 Range 校验内容，遇到可恢复的上游错误时通过 acquire feedback/exclude 最多轮换四个授权（包含原授权），并通过 report 回报终态。
     - upstream `4xx`/`5xx` terminal response 会先取消 upstream body，再返回 Worker-generated JSON `{ status, message, reason }`，并按需附带 `upstream_status` 与 `retry-after`；CQ managed 与非 CQ 路径都共享这一 terminal matrix。
...
- 下载缓存：`DOWNLOAD_CACHE_TABLE` + `DOWNLOAD_CACHE_REFRESH` + `download_get_cache_state` + `download_acquire_cache_refresh` + `download_finish_cache_refresh` + `download_cleanup_expired_cache`
...
- 业务策略来自控制面下发的 bootstrap/decision。
